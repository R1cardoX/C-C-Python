...
class SLICProcessor(object):
        ...
        for cluster in self.clusters:
            ...
            for dh in range(-1, 2):
                for dw in range(-1, 2):
                    ...
                    _w = cluster.w + dw
                    print(str(_h)+'   '+str(_w))
                    print(str(cluster.h)+','+str(cluster.w))
    ...
    def iterate_10times(self):
        ...
        for i in trange(10):
            self.assignment()
            self.update_cluster()
            name = ('lenna_M{m}_K{k}_loop{loop}.'+self.filename.split('.')[1]).format(loop=i, m=self.M, k=self.K)
            self.save_current_image(name)


if __name__ == '__main__':
    p = SLICProcessor(sys.argv[1], int(sys.argv[2]), int(sys.argv[3])) #argv1 image name,argv2(200-1000),argv3(5-40)
    p.iterate_10times()
